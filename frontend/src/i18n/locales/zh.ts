export default {
  ...
  userAccounts: {
    ...
    importWarning:
      "单次最多导入 {max} 个账号。支持 CCAPI OAuth JSON、Codex-Manager ChatGPT Token JSON、OpenAI Refresh Token、Claude Session Key；API Key、URL、Upstream、Cookie 会被拒绝。",
    ...
    importWarningClaude:
      "单次最多导入 {max} 个 Claude 账号。支持 CCAPI OAuth JSON 或 Claude Session Key。",
    importWarningGemini:
    ...
    importErrors: "导入明细",
    importInvalidFile:
      "导入文件无效：需要账号数组或包含 accounts 的 CCAPI 导出文件。",
  ...
  admin: {
    ...
    accounts: {
      ...
      credentialImportWarning:
		"该入口只接收官方 OAuth 凭证与 OpenAI Agent Identity JSON：CCAPI OAuth JSON、Codex-Manager ChatGPT Token JSON、OpenAI Refresh Token、Claude Session Key；API Key、URL、Upstream、Cookie 会被拒绝。Agent Identity 仅支持管理员私有账号，不支持归属用户或公开共享。",
    ...
    settings: {
      ...
      openaiFastPolicy: {
        ...
        userIds: "指定用户",
        userIdsHint:
          "输入任意邮箱关键词进行模糊搜索。留空表示对全部 CCAPI 用户生效；选中用户的 API Key 请求优先匹配用户规则。",
