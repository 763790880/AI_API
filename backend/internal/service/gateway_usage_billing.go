...
import (
	...
	"github.com/your-org/ai-gateway-platform/internal/pkg/ctxkey"
	"github.com/your-org/ai-gateway-platform/internal/pkg/logger"
	"github.com/your-org/ai-gateway-platform/internal/pkg/timezone"
)
...
type postUsageBillingParams struct {
	...
	UsageLogID            int64  // 分账幂等键：本请求落库后的 usage_log_id（0 = 未捕获/无需分账）
}
...
// shouldShareCredit 判断本次计费是否需要对托管账号做分账入账。
func shouldShareCredit(p *postUsageBillingParams) bool {
	return p != nil && p.Cost != nil && p.Account != nil &&
		p.Account.OwnerUserID != nil && *p.Account.OwnerUserID != 0 &&
		p.Cost.ActualCost > 0
}
...
func buildUsageBillingCommand(requestID string, usageLog *UsageLog, p *postUsageBillingParams) *UsageBillingCommand {
	...
	if p.Account.OwnerUserID != nil {
		cmd.OwnerUserID = *p.Account.OwnerUserID
	}
	if usageLog != nil {
...
func applyUsageBilling(ctx context.Context, requestID string, usageLog *UsageLog, p *postUsageBillingParams, deps *billingDeps, repo UsageBillingRepository) (bool, error) {
	...
	// 分账幂等需要 usage_log_id，但 usage_log 在计费成功后才落库（recordUsageCore
	// 在 applyUsageBilling 返回后写）。这里对托管账号同步补写一次以捕获 ID；
	// usage_logs 有 (request_id, api_key_id) 唯一约束，重复写是幂等 no-op。
	if shouldShareCredit(p) && usageLog != nil && usageLog.ID == 0 && deps.usageLogRepo != nil {
		if _, writeErr := deps.usageLogRepo.Create(billingCtx, usageLog); writeErr != nil {
			logger.LegacyPrintf("service.gateway", "capture usage log id for share credit failed: %v", writeErr)
		}
	}
	if usageLog != nil {
		p.UsageLogID = usageLog.ID
	}
...
func finalizePostUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	// 分账入账钩子：托管账号被调用并计费成功后，异步按 ActualCost × 分成比例写入号主收益账本。
	// 幂等键为 usage_log_id（owner_ledger 的 (ref_type, ref_id) 部分唯一索引）。
	// 失败只记 ALERT 日志，不阻塞主请求。
	if shouldShareCredit(p) && p.UsageLogID > 0 && deps.settlementService != nil {
		dbCtx, dbCancel := detachUpstreamContext(ctx)
		ownerID := *p.Account.OwnerUserID
		usageLogID := p.UsageLogID
		consumerCharge := p.Cost.ActualCost
		go func() {
			defer func() {
				if r := recover(); r != nil {
					logger.LegacyPrintf("service.gateway", "ALERT: panic in share credit goroutine owner=%d usage_log=%d: %v", ownerID, usageLogID, r)
				}
			}()
			defer dbCancel()
			if err := deps.settlementService.RecordShareCredit(dbCtx, ownerID, usageLogID, consumerCharge); err != nil {
				logger.LegacyPrintf("service.gateway", "ALERT: record share credit failed owner=%d usage_log=%d charge=%f: %v", ownerID, usageLogID, consumerCharge, err)
			}
		}()
	}
}
...
func (s *GatewayService) billingDeps() *billingDeps {
	return &billingDeps{
		...
		settlementService:     s.settlementService,
		cfg:                   s.cfg,
	}
}
