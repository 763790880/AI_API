...
func (User) Fields() []ent.Field {
	return []ent.Field{
		...
		// can_host_accounts: 是否具备账号托管（号主）权限。仅开启后该用户可托管账号并参与分账。
		field.Bool("can_host_accounts").
			Default(false).
			Comment("Whether the user can host accounts and participate in revenue sharing."),
	}
}
...
func (User) Edges() []ent.Edge {
	return []ent.Edge{
		...
		// owned_accounts: 该用户作为号主托管的账号集合（反向边为 Account.owner）。
