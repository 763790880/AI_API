...
func (Account) Fields() []ent.Field {
	return []ent.Field{
		...
		// ========== 账号托管共享字段（语义移植自 PixelAPI） ==========
		// owner_user_id: 账号归属用户（号主）ID。NULL 表示平台自有账号。
		field.Int64("owner_user_id").
			Optional().
			Nillable().
			Comment("Owner user id for account hosting (NULL = platform-owned)."),
		// share_mode: 共享模式。private=仅号主；public=平台内共享；plaza=广场房间。
		field.String("share_mode").
			MaxLen(20).
			Default(domain.ShareModePrivate).
			Comment("Account share mode: private/public/plaza."),
		// share_status: 共享审核状态。pending=待审核；approved=已通过；rejected=已拒绝。
		field.String("share_status").
			MaxLen(20).
			Default(domain.ShareStatusPending).
			Comment("Account share approval status: pending/approved/rejected."),
		// share_policy_id: 显式绑定的分账策略 ID。NULL 表示按作用域自动解析。
		field.Int64("share_policy_id").
			Optional().
			Nillable().
			Comment("Explicit share policy id override (NULL = resolve by scope)."),
		// account_level: 账号能力等级键（如 plus/pro/enterprise），用于分层计费/调度。
		field.String("account_level").
			MaxLen(64).
			Optional().
			Comment("Account capability level key (e.g. plus/pro/enterprise)."),
		// max_seats: 该账号最大并发共享座位数。
		field.Int("max_seats").
			Default(1).
			Comment("Max concurrent seats for account sharing."),
...
func (Account) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.To("proxy", Proxy.Type).
			Field("proxy_id").
			Unique(),
		// owner: 账号归属用户（号主），反向边为 User.owned_accounts。
...
func (Account) Indexes() []ent.Index {
	return []ent.Index{
		...
		// 账号托管共享查询索引
		index.Fields("owner_user_id"),              // 按号主筛选
		index.Fields("share_mode", "share_status"), // 按共享模式+审核状态筛选
	}
}
