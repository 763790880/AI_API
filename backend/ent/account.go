...
type Account struct {
	...
	// Owner user id for account hosting (NULL = platform-owned).
	OwnerUserID *int64 `json:"owner_user_id,omitempty"`
	// Account share mode: private/public/plaza.
	ShareMode string `json:"share_mode,omitempty"`
	// Account share approval status: pending/approved/rejected.
	ShareStatus string `json:"share_status,omitempty"`
	// Explicit share policy id override (NULL = resolve by scope).
	SharePolicyID *int64 `json:"share_policy_id,omitempty"`
	// Account capability level key (e.g. plus/pro/enterprise).
	AccountLevel string `json:"account_level,omitempty"`
	// Max concurrent seats for account sharing.
...
func (*Account) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case account.FieldID, account.FieldOwnerUserID, account.FieldSharePolicyID, account.FieldMaxSeats, account.FieldProxyID, account.FieldProxyFallbackOriginID, account.FieldConcurrency, account.FieldLoadFactor, account.FieldPriority, account.FieldParentAccountID:
			values[i] = new(sql.NullInt64)
		case account.FieldName, account.FieldNotes, account.FieldPlatform, account.FieldType, account.FieldShareMode, account.FieldShareStatus, account.FieldAccountLevel, account.FieldStatus, account.FieldErrorMessage, account.FieldTempUnschedulableReason, account.FieldSessionWindowStatus, account.FieldQuotaDimension:
...
func (_m *Account) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case account.FieldShareMode:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field share_mode", values[i])
			} else if value.Valid {
				_m.ShareMode = value.String
			}
		case account.FieldShareStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field share_status", values[i])
			} else if value.Valid {
				_m.ShareStatus = value.String
			}
		case account.FieldSharePolicyID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field share_policy_id", values[i])
			} else if value.Valid {
				_m.SharePolicyID = new(int64)
				*_m.SharePolicyID = value.Int64
			}
		...
		case account.FieldMaxSeats:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field max_seats", values[i])
			} else if value.Valid {
				_m.MaxSeats = int(value.Int64)
			}
...
func (_m *Account) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("share_mode=")
	builder.WriteString(_m.ShareMode)
	builder.WriteString(", ")
	builder.WriteString("share_status=")
	builder.WriteString(_m.ShareStatus)
	builder.WriteString(", ")
	if v := _m.SharePolicyID; v != nil {
		builder.WriteString("share_policy_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	...
	builder.WriteString(", ")
	builder.WriteString("max_seats=")
	builder.WriteString(fmt.Sprintf("%v", _m.MaxSeats))
	builder.WriteString(", ")
